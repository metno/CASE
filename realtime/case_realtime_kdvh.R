#!/usr/bin/env Rscript
# << case_realtime_kdvh.R >>
# Create files with the KDVH data for realtime applications.
# The data are downloaded in different files depending on: countries (Norway
# or foreign data), WMO (yes/no), MET Norway among the station holders (yes/no)
#------------------------------------------------------------------------------
t0<-Sys.time()
#------------------------------------------------------------------------------
# load libraries
...
if (is.na(argv$elcode_hourly)) 
  argv$elcode_hourly<-c("FF","PR","TA","DG","DD","RR_1","FG","UU")
if (is.na(argv$elcode_diurnal)) 
  argv$elcode_diurnal<-c("TAMRR","TAM","RR","TAN","TAX")
if (is.na(argv$dqc_ok)) 
  argv$dqc_ok<-c(0,1,2,NA)
#-----------------------------------------------------------------------------
# cristianl@met.no client ID
